...
export default function SongCard({ track, index, queue, showIndex }: Props) {
  ...
  return (
    ...
    >
      {showIndex && (
        <span className="w-6 text-center text-xs text-zinc-500 font-mono">{index! + 1}</span>
      )}

      <div className="relative w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-gradient-to-br from-zinc-700 to-zinc-900 album-hover">
        {track.album.cover_medium || track.youtubeId ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <svg className="w-5 h-5 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
        ...
        <div className="play-overlay absolute inset-0 flex items-center justify-center bg-black/30 backdrop-blur-[2px]">
          <div className="w-8 h-8 rounded-full bg-[#D4AF37]/90 flex items-center justify-center gold-glow-btn">
      ...
      <div className="min-w-0 flex-1">
        <p className={`text-sm font-medium truncate ${isCurrentTrack ? 'text-[#D4AF37]' : 'text-white'}`}>
          {track.title}
        </p>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="w-7 h-7 rounded-full bg-white/5 hover:bg-[#D4AF37]/20 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
          title="Add to queue"
        >
        ...
        <button
          ...
          className="w-8 h-8 rounded-full bg-white/5 hover:bg-[#D4AF37]/20 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
        >
        ...
        <button
          ...
          disabled={dl}
          className="w-7 h-7 rounded-full bg-white/5 hover:bg-[#D4AF37]/20 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100 disabled:opacity-50"
          title="Download"
        >
          {dl ? (
